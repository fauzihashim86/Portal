...
if(isset($_POST['func'])) {
  ...
  } else if($function == "saveAdmin") {
    $admin = "";
    if($_POST['task'] == "system"){
      $admin = array($_POST['task'],$_POST['id'],escapeString($_POST['system']),escapeString($_POST['url']),escapeString($_POST['database']));
    } else if($_POST['task'] == "dms"){
      $admin = array($_POST['task'],$_POST['id'],escapeString($_POST['system']),escapeString($_POST['url']));
    } else  if($_POST['task'] == "phone"){
      $admin = array($_POST['task'],$_POST['id'],escapeString($_POST['name']),escapeString($_POST['ext']),escapeString($_POST['hp']),$_POST['dept'],$_POST['level']);
    } else if($_POST['task'] == "level"){
      $admin = array($_POST['task'],$_POST['id'],escapeString($_POST['level']));
    } else if($_POST['task'] == "division"){
      $admin = array($_POST['task'],$_POST['id'],escapeString($_POST['division']));
    } else if($_POST['task'] == "department"){
      $admin = array($_POST['task'],$_POST['id'],$_POST['division'],escapeString($_POST['department']));
    } else if($_POST['task'] == "attach"){
      $admin = array($_POST['task'],$_POST['id'],$_FILES['file']);
    } else if($_POST['task'] == "form" || $_POST['task'] == "forms"){
      $admin = array($_POST['task'],$_POST['id'],$_POST['desc']);
    } else if($_POST['task'] == "attachForm" || $_POST['task'] == "attachmentForm"){
      if(isset($_FILES['file'])){
        $admin = array($_POST['task'],$_POST['id'],$_POST['desc'],$_FILES['file']);
      } else {
        $admin = array($_POST['task'],$_POST['id'],$_POST['desc']);
      }
    }
...
function saveAdmin($admin){
  if($admin[0] == "system"){
    if(!$admin[1]){
      $query = "INSERT INTO e_central.system(sys_title,sys_db,sys_url)VALUES('$admin[2]','$admin[3]','$admin[4]')";
    } else {
      $query = "UPDATE e_central.system SET sys_title='$admin[2]',sys_db='$admin[3]',sys_url='$admin[4]' WHERE sys_id=$admin[1]";
    }
  } else if($admin[0] == "dms"){
    if(!$admin[1]){
      $query = "INSERT INTO e_central.dms(dms_desc,dms_url)VALUES('$admin[2]','$admin[3]')";
    } else {
      $query = "UPDATE e_central.system SET dms_desc='$admin[2]',dms_url='$admin[3]' WHERE dms_id=$admin[1]";
    }
  } else if($admin[0] == "phone"){
    if(!$admin[1]){
      $query = "INSERT INTO phone_dir.phone(emp_name,emp_ext,emp_hp,emp_dept,emp_level)VALUES('$admin[2]','$admin[3]','$admin[4]',$admin[5],$admin[6])";
    } else {
      $query = "UPDATE phone_dir.phone SET emp_name='$admin[2]',emp_ext='$admin[3]',emp_hp='$admin[4]',emp_dept=$admin[5],emp_level=$admin[6] WHERE dms_id=$admin[1]";
    }
  } else if($admin[0] == "level"){
    if(!$admin[1]){
      $query = "INSERT INTO phone_dir.level(lvl_name)VALUES('$admin[2]')";
    } else {
      $query = "UPDATE phone_dir.level SET lvl_name='$admin[2]' WHERE lvl_id=$admin[1]";
    }
  } else if($admin[0] == "division"){
    if(!$admin[1]){
      $query = "INSERT INTO phone_dir.division(div_name)VALUES('$admin[2]')";
    } else {
      $query = "UPDATE phone_dir.division SET div_name='$admin[2]' WHERE div_id=$admin[1]";
    }
  } else if($admin[0] == "department"){
    if(!$admin[1]){
      $query = "INSERT INTO phone_dir.department(div_id,dept_name)VALUES($admin[2],'$admin[3]')";
    } else {
      $query = "UPDATE phone_dir.department SET div_id=$admin[2],dept_name='$admin[3]' WHERE dept_id=$admin[1]";
    }
  } else if($admin[0] == "attach"){
    if(isset($admin[2])){
      $target_dir = "../upload/pdf/".$admin[2]['name'];
      $new_name = $admin[2]['name'];
      if(file_exists($target_dir)) {
        // unlink($target_dir);
        $new_name = renames($admin[2]['name']);
        $target_dir = "../upload/pdf/".$new_name;
      }
      if(move_uploaded_file($admin[2]["tmp_name"],$target_dir)){
        $query = "UPDATE e_central.policy SET p_filename='$new_name' WHERE p_id=$admin[1]";
      }
    }
    $admin[0] = "policy";
  } else if($admin[0] == "form" || $admin[0] == "forms"){
    if(!$admin[1]){
      $query = "INSERT INTO e_central.ref_category(rc_type,rc_desc)VALUES('$admin[0]','$admin[2]')";
    } else {
      $query = "UPDATE e_central.ref_category SET rc_desc='$admin[2]' WHERE rc_id=$admin[1]";
    }
    $admin[0] = "form";
  } else if($admin[0] == "attachmentForm"){
  ...
  <tr class="text-center">
    ...
    <td>
      <div class="dropdown">
        ...
        <div class="dropdown-menu dropdown-menu-right dropdown-menu-icon-list">
          <a class="dropdown-item" href="upload/pdf/<?php echo $row['p_filename'];?>" target="_blank"><i class="dw dw-eye"></i> View</a>
...
function admin($id=null,$task=null) {
  echo "<input type='hidden' id='task_id' value='".$id."'>";
  if($task == "system"){
    $name = $db = $url = "";
    if($id != null){
      $result = executeQuery("SELECT * FROM e_central.system WHERE sys_id=$id");
      $row = $result->fetch();
      $name = $row['sys_title'];
      $db = $row['sys_db'];
      $url = $row['sys_url'];
  ...
  <div class="row">
    <div class="col-12 form-group">
  		<label>System <sup class="text-danger">*</sup></label>
  ...
  <div class="row">
      <div class="col-12 form-group">
  		<label>Database <sup class="text-danger">*</sup></label>
  		<input class="form-control" id="database" type="text" value="<?php echo $db?>">
  	</div>
  </div>
  <?php } else if($task == "phone"){
    $name = $ext = $hp = $dept = $level = "";
    if($id != null){
      $result = executeQuery("SELECT * FROM phone_dir.phone WHERE emp_id=$id");
      $row = $result->fetch();
      $name = $row['emp_name'];
      $ext = $row['emp_ext'];
      $hp = $row['emp_hp'];
  ...
  <div class="row">
    <div class="col-6 form-group">
      <label>Extension <sup class="text-danger">*</sup></label>
      <input class="form-control" id="ext" type="text" value="<?php echo $ext;?>">
    </div>
    <div class="col-6 form-group">
      <label>Mobile</label>
      <input class="form-control" id="hp" type="text" value="<?php echo $hp?>">
    </div>
  </div>
  <div class="row">
      <div class="col-12 form-group">
      <label>Department <sup class="text-danger">*</sup></label>
      <select class="form-control" style="width: 100%;" id="department_form">
        <option value="0">-- Please Select --</option>
  			<?php selectDepartDivision($dept);?>
      </select>
    </div>
  </div>
  <div class="row">
      ...
      <select class="form-control" style="width: 100%;" id="level_form">
        <option value="0">-- Please Select --</option>
  			<?php selectLevel($level);?>
      </select>
    </div>
  </div>
<?php } else if($task == "level"){
    $name = "";
    if($id != null){
      $result = executeQuery("SELECT * FROM phone_dir.level WHERE lvl_id=$id");
      $row = $result->fetch();
      $name = $row['lvl_name'];
    }
  ?>
  <div class="row">
    <div class="col-12 form-group">
      <label>Name <sup class="text-danger">*</sup></label>
      <input class="form-control" id="level_form" type="text" value="<?php echo $name?>">
    </div>
  </div>
<?php } else if($task == "division"){
      $name = "";
      if($id != null){
        $result = executeQuery("SELECT * FROM phone_dir.division WHERE div_id=$id");
        $row = $result->fetch();
        $name = $row['div_name'];
      }
    ?>
    <div class="row">
      <div class="col-12 form-group">
        <label>Name <sup class="text-danger">*</sup></label>
        <input class="form-control" id="division_form" type="text" value="<?php echo $name?>">
      </div>
    </div>
  <?php } else if($task == "department"){
        $name = $div = "";
        if($id != null){
          $result = executeQuery("SELECT * FROM phone_dir.department WHERE dept_id=$id");
          $row = $result->fetch();
          $div = $row['div_id'];
          $name = $row['dept_name'];
        }
      ...
      <div class="row">
        <div class="col-12 form-group">
          ...
          <select class="form-control" style="width: 100%;" id="division_form">
            <option value="0">-- Please Select --</option>
      			<?php selectDivision($div);?>
          </select>
        </div>
      </div>
      <div class="row">
        <div class="col-12 form-group">
          <label>Name <sup class="text-danger">*</sup></label>
          <input class="form-control" id="department_form" type="text" value="<?php echo $name?>">
        </div>
      </div>
  <?php } else if($task == "dms"){
    $name = $url = "";
    if($id != null){
      $result = executeQuery("SELECT * FROM e_central.dms WHERE dms_id=$id");
      $row = $result->fetch();
      $name = $row['dms_desc'];
      $url = $row['dms_url'];
    }
...
<?php } else if($task == "attach"){
  $result = executeQuery("SELECT * FROM e_central.policy WHERE p_id=$id");
  $row = $result->fetch();
  ?>
  <div class="row">
    <div class="col-12">
  		<embed src="upload/pdf/<?php echo $row['p_filename'];?>" width="100%" height="450" type="application/pdf">
  	</div>
  </div>
  <div class="row">
    <div class="col-6 form-group">
      <label>Description <sup class="text-danger">*</sup></label>
      <input type="text" class="form-control" id="desc" value="<?php echo $row['p_desc'];?>">
    </div>
    <div class="col-6 form-group">
  		<label>Upload</label>
  ...
  <?php
    ...
    $result = executeQuery("SELECT rc_desc FROM ref_category WHERE rc_id=$id");
    $row = $result->fetch();
    ?>
    <div class="row">
      <div class="col-12 form-group">
        <label>Category <sup class="text-danger">*</sup></label>
        <input type="text" class="form-control" id="desc" value="<?php echo $row['rc_desc'];?>">
      </div>
    </div>
    <?php
  } else if($task == "policy"){
    ...
    <div class="row">
      <div class="col-12 form-group">
        <label>Category <sup class="text-danger">*</sup></label>
        <input type="text" class="form-control" id="desc" value="<?php echo $row['rc_desc'];?>" readonly>
      </div>
    </div>
    <div class="row">
      <div class="col-12 form-group">
    ...
    <div class="row">
      <div class="col-12 form-group">
        <label>File</label><br>
        <a href="upload/pdf/<?php echo $row['f_filename'];?>" target="_blank" class="text-blue"><i class="fa fa-file"></i> <?php echo $row['f_filename'];?></a>
      </div>
    </div>
    <div class="row">
      <div class="col-12 form-group">
        <label>Description <sup class="text-danger">*</sup></label>
        <input type="text" class="form-control" id="desc" value="<?php echo $row['f_desc'];?>">
      </div>
    </div>
    <div class="row">
      <div class="col-12 form-group">
...
?>
  <div class="row"><div class="col-12"><small><sup class="text-danger">*</sup> required field.</small></div></div>
<?php }
function getTable($table=null){
  ...
  <div class="row" style="overflow-x: scroll;">
    <div class="col-12">
    <?php if($table == "system"){?>
      <table id="table_<?php echo $table;?>" class="data-table table stripe hover nowrap">
  			<thead>
  				<tr>
          ...
          ?>
            <tr class="text-center">
              ...
              <td class="text-left"><?php echo $row['sys_url'];?></td>
            ...
            <tr class="text-center">
              ...
              <td>
                <div class="dropdown">
  								<a class="btn btn-sm btn-success dropdown-toggle" href="#" role="button" data-toggle="dropdown">Action</a>
  								<div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('policyNew',<?php echo $row['rc_id'];?>)">New Sub</a>
                    <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('attachPolicy',<?php echo $row['rc_id'];?>)">New Policy</a>
  									<a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('policys',<?php echo $row['rc_id'];?>)">Edit</a>
  								</div>
  							</div>
              </td>
            ...
            <tr class="text-center">
              ...
              <td>
                <div class="dropdown">
  								<a class="btn btn-sm btn-success dropdown-toggle" href="#" role="button" data-toggle="dropdown">Action</a>
  								<div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('attachmentForm',<?php echo $row['rc_id'];?>)">New Form</a>
                    <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('forms',<?php echo $row['rc_id'];?>)">Edit</a>
  								</div>
  							</div>
              </td>
              <td class="text-left"><b><?php echo $row['rc_desc'];?></b></td>
            ...
            <tr class="text-center">
              ...
              <td>
                <div class="dropdown">
  								<a class="btn btn-sm btn-success dropdown-toggle" href="#" role="button" data-toggle="dropdown">Action</a>
  								<div class="dropdown-menu dropdown-menu-right">
                    <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('<?php echo $table;?>',<?php echo $row['emp_id'];?>)">Edit</a>
  								</div>
  							</div>
              </td>
              <td class="text-left"><?php echo $row['emp_name'];?></td>
          ...
          <div class="dropdown">
            ...
            <div class="dropdown-menu dropdown-menu-right">
              <?php if(!$row['p_filename']){?>
                <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('newCategory',<?php echo $row['p_rc_id'];?>)">New Sub</a>
                <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('attachmentPolicy',<?php echo $row['p_id'];?>)">New Policy</a>
                <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('category',<?php echo $row['p_id'];?>)">Edit</a>
              <?php } else {?>
                <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('attach',<?php echo $row['p_id'];?>)">Edit</a>
              <?php }?>
            </div>
          </div>
        </td>
        <td class="text-left"><?php if(!$row['p_filename']){ echo "<p class='text-green'><b>".$row['p_desc']." <i class='fa fa-arrow-down'></i></b></p>";} else { echo "<a href='upload/pdf/".$row['p_filename']."' target='_blank' class='text-blue'>".$row['p_desc']."</a>";}?></td>
        <td>
          <?php
            $class = "";
            if($row['p_active'] == 1) {
...
function subForm($count,$id){
  $c = 1;
  $result = executeQuery("SELECT * FROM e_central.forms WHERE f_rc_id=$id ORDER BY f_sort");
  if($result->rowCount()> 0){
    while($row = $result->fetch()){
      ?>
      <tr class="text-center">
        ...
        <td>
          <div class="dropdown">
            <a class="btn btn-sm btn-success dropdown-toggle" href="#" role="button" data-toggle="dropdown">Action</a>
            <div class="dropdown-menu dropdown-menu-right">
              <a class="dropdown-item" style="cursor:pointer;" onclick="adminModal('attachForm',<?php echo $row['f_id'];?>)">Edit</a>
            </div>
          </div>
        </td>
        <td class="text-left"><?php if(!$row['f_filename']){ echo "<p class='text-green'><b>".$row['f_desc']." <i class='fa fa-arrow-down'></i></b></p>";} else { echo "<a href='upload/pdf/".$row['f_filename']."' target='_blank' class='text-blue'>".$row['f_desc']."</a>";}?></td>
        <td>
          <?php
            $class = "";
            if($row['f_active'] == 1) {
          ...
          ?><input type="checkbox" id="form_<?php echo $row['f_id'];?>" <?php echo $class;?> class="switch-btn" data-color="#28a745" data-secondary-color="red" data-size="small" onchange="updateStatus('form',<?php echo $row['f_id'];?>)">
        </td>
      </tr>
      <?php
      //if(!$row['p_filename']){ subTR($count,$row['p_id']);}
...
function phoneUser(){
  $result = executeQuery("SELECT user_id FROM phone_dir.user WHERE user_name='".$_SESSION["login"]["username"]."'");
  return $result->rowCount();
}
function countToll(){
...
function changeView($sys=0) {
  ...
  <?php if($row['sys_id'] <> 0 && $row['sys_id'] <> 9) {?>
  ...
  if($row['sys_id'] == "9" && phoneUser() > 0) {
    ?>
    <div class="col-12 mb-30">
      Click <a href="./?sub=4" style="color:blue;text-decoration-line: underline;cursor:pointer;"><b>here</b></a> to access Administrator module.
    </div>
    <?php
  }
  if($sys == "0") {
          ...
          <div class="card-body">
            <?php
              ...
              if($policy->rowCount() == 1) {
                $pol = $policy->fetch();
                ?><embed src="upload/pdf/<?php echo $pol['p_filename'];?>" width="100%" height="1150" type="application/pdf"><?php
              } else {
                ?>
                <div class="tab">
                          ...
                          while($content = $body->fetch()) {
                            if(!$content['p_filename']) {
                              ...
                              <div class="<?php echo $aria;?>" id="tab<?php echo $content['p_id'];?>" role="tabpanel">
        												<div class="pd-20">
                                  <embed src="upload/pdf/<?php echo $content['p_filename'];?>" width="100%" height="1150" type="application/pdf">
                                </div>
        											</div>
                              <?php
                            }
    ...
    <div class="col-12">
      <button id="search" class="btn btn-success pull-right" onclick="searchDirectory()"><i class="fa fa-search"></i> Search</button>
    </div>
...
function updateStatus($task,$id,$status) {
  ...
  if($task == "system"){
    ...
    executeQuery("UPDATE e_central.forms SET f_active='$val' WHERE f_id='$id'");
  }
}
...
function selectDepartDivision($id=null) {
  ...
  if($result->rowCount() != 0) {
    while($row = $result->fetch()) {
      ...
      if($id != null){
        $results = executeQuery("SELECT dept_id,dept_name FROM phone_dir.department WHERE div_id=$div_id");
      }
      if($results->rowCount() != 0) {
        ?>
        <optgroup label="<?php echo $row['div_name'];?>">
          <?php while($rows = $results->fetch()) {?>
            <option value="<?php echo $rows['dept_id'];?>" <?php if($id == $rows['dept_id']) {echo "selected";}?>><?php echo $rows['dept_name'];?></option>
          <?php }?>
        </optgroup>
        <?php
      }
...
function selectLevel($id=null) {
  ...
  if($result->rowCount() != 0) {
    while($row = $result->fetch()) {
    ?><option value="<?php echo $row['lvl_id'];?>" <?php if($id == $row['lvl_id']) {echo "selected";}?>><?php echo $row['lvl_name'];?></option><?php
    }
  }
}
function selectDivision($id=null) {
  $result = executeQuery("SELECT div_id,div_name FROM phone_dir.division ORDER BY div_name");
  if($result->rowCount() != 0) {
    while($row = $result->fetch()) {
    ?><option value="<?php echo $row['div_id'];?>" <?php if($id == $row['div_id']) {echo "selected";}?>><?php echo $row['div_name'];?></option><?php
    }
  }
}
  ...
  while($row = $result->fetch()) {
    if(!$row['p_filename']) {
      ...
      <div class="<?php echo $aria;?>" id="tab<?php echo $row['p_id'];?>" role="tabpanel">
        <div class="pd-20">
          <embed src="upload/pdf/<?php echo $row['p_filename'];?>" width="100%" height="1150" type="application/pdf">
        </div>
      </div>
      <?php
    }
